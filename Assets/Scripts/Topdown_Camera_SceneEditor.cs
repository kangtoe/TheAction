...
public class Topdown_Camera_SceneEditor : Editor
{
    ...
    {        
        if(!targetCamera) targetCamera = (TopdownCamera)target;

        // Make sure we have a target first!
        if (!targetCamera || !targetCamera.target)
        {
            if (!targetCamera) Debug.Log("!targetCamera");
            else if (!targetCamera.target) Debug.Log("!targetCamera.target");
